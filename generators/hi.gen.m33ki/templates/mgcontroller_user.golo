  ...
  return DynamicObject()
    : define("getAll", |this, request, response| {
      # GET request : get all models
        response: json(Json(): toJsonString(<%= infos: modelName(): toLowerCase() %>s: fetch())): status(200)
    })
    ...
    : define("create", |this, request, response| {
      # POST request : create a model
        ...
        <%= infos: modelName(): toLowerCase() %>: encryptPwd()
        <%= infos: modelName(): toLowerCase() %>: setStdRights()
